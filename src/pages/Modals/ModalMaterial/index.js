...
const ModalMaterial = props => {
  const {
    dataModal: {
      modalType,
      modalShow,
      modalData: { type, handleSave, value, max },
    },
    dispatch,
    imgMaterialList,
    vedioMaterialList,
  ...
  useEffect(() => {
    if (modalShow && modalType === 'MODAL_MATERIAL') {
      setParams(def);
    }
  }, [modalShow, modalType]);

  useEffect(() => {
    if (modalShow && modalType === 'MODAL_MATERIAL' && value) {
      setSelects(value);
    }
  }, [value]);
  ...
  const handleUploadVideoSave = val => {
    dispatch({
      type: 'sourceHouse/materialSave',
      payload: {
        params: val,
        reload: () => {
          setParams(def);
          dispatch({ type: 'sourceHouse/closeModal' });
        },
      },
    });
  };
  ...
  return (
    <Modal
      ...
      zIndex="1000"
      title={type === 1 ? '我的图片' : '我的视频'}
      width="900px"
      maskClosable={false}
      visible={modalType === 'MODAL_MATERIAL' && modalShow}
      onCancel={() => handleCancel()}
      onOk={() => handleOk()}
    >
      <ModalUploadVideo handleSave={handleUploadVideoSave} />
...
export default connect(mapStateToProps)(ModalMaterial);
